...
void alignDirectory(string dirname, vector<string> filelist, int numframes, int numaligned){
	vector<int> compression_params;
	 ...
	 if (numaligned ==0){
		...
		{
    		size_t dotPos = filelist[0].rfind(".png");
    		if (dotPos != string::npos) filelist[0].insert(dotPos, "_aligned");	
		}
imwrite(filelist[0], frame_gray, compression_params );
{
    // Recover original (raw) name by stripping "_aligned"
    std::string raw0 = filelist[0];
    size_t dot = raw0.rfind(".png");
    size_t tag = (dot == std::string::npos) ? std::string::npos
                                            : raw0.rfind("_aligned", dot);
    if (tag != std::string::npos) {
        raw0.erase(tag, std::string("_aligned").size());
        std::cout << "timestamp" << swapTimes(raw0, filelist[0]) << std::endl;
    }
}
	...
	for (int i=numaligned; i < numframes; i++){
				...
				try{
						...
						//frame is bad...overwrite it with the previous frame
							...
							string target = filelist[i];
							{
    							size_t dotPos = target.rfind(".png");
    							if (dotPos != string::npos) target.insert(dotPos, "_aligned");
							}
							outss << " " << target << endl;
	...
	*/				
				...
				//end warp ECC
				 ...
				 Mat im2_aligned_gray;
					...
					{
    					size_t dotPos = filelist[i].rfind(".png");
    					if (dotPos != string::npos) filelist[i].insert(dotPos, "_aligned");
					}
				...
				//Write timestamps with openCV
					...
					// derive frame number from the original name "oldname"
					string fileframenumber;
					{
    					size_t fpos = oldname.rfind("_frame");
    					if (fpos != string::npos) {
        				size_t start = fpos + 6; // length of "_frame"
        				size_t end = oldname.find('.', start);
        				if (end == string::npos) end = oldname.length();
        				fileframenumber = oldname.substr(start, end - start);
    				} else {
        				fileframenumber = ""; // fallback if pattern not found
    				}
					}
...
void readDirectory(string fulldirectory){
	...
	globpattern  << fulldirectory.c_str() << string("day*_image*_frame*.png");
	...
	// scan for aligned files and build a set of their raw counterparts
	globpattern.str("");
	globpattern << fulldirectory.c_str() << string("*_aligned.png");
	glob(globpattern.str().c_str(), GLOB_TILDE, NULL, &aligned_result);

	// Build a set of fully-qualified raw names by stripping "_aligned" before ".png"
	std::unordered_set<std::string> already_aligned;
	for (unsigned int i = 0; i < aligned_result.gl_pathc; i++) {
	    std::string p = aligned_result.gl_pathv[i];
	    size_t dot = p.rfind(".png");
	    size_t tag = (dot == std::string::npos) ? std::string::npos
	                                            : p.rfind("_aligned", dot);
	    if (tag != std::string::npos) {
	        p.erase(tag, std::string("_aligned").size()); // remove suffix
	        already_aligned.insert(p);
	    }
	}

	// Build a worklist of raw frames that still need alignment
	std::vector<std::string> todo;
	todo.reserve(filelist.size());
	for (const auto &raw : filelist) {
	    if (!already_aligned.count(raw)) {
	        todo.push_back(raw);
	    }
	}
	...
	//sort lexicographically for stable processing order
	std::sort(todo.begin(), todo.end());
	alignDirectory(fulldirectory, todo, (int)todo.size(), /*numaligned=*/0);
