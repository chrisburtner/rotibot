...
// Description : creates a CSV log of file creation times for a folder
//============================================================================
...
string getFrameNumber(string numfile){

	string start = "frame";
        string end = ".png";

    string::size_type pos = numfile.find( start );
    pos = ( pos == string::npos ? 0 : pos + start.size() );

    string::size_type n = numfile.rfind( end );

    n = ( n == string::npos || n < pos ? string::npos : n - pos );

    string noextension(numfile.substr( pos, n ));
	
    return noextension;

}//end getframnumner

string getFrameTime(string timefile) {

	stringstream lineoutput; //hold the line

	string framenum = getFrameNumber(timefile);
	
	long creation = getFileCreationTime(timefile);
	lineoutput << framenum << "," << creation;

	return lineoutput.str();
...
void readDirectory(string fulldirectory){
	...
	logname << fulldirectory << "timelog.csv";
	ofstream timelog(logname.str().c_str());
